...
public class Test {
    public static void main(String[] args) {
        ...
        List<Human> filteredHumans = humans.stream().filter(h -> h.getAge() > 20).toList();
        filteredHumans.forEach(System.out::println);
        System.out.println("-----------------------------");
        System.out.println("Sorted for last char of name:");
        List<Human> sortedHumans = filteredHumans.stream().sorted(new Comparator<Human>() {
            @Override
        ...
        List<Human> newAgeHumans = sortedHumans.stream().peek(h -> h.setAge(h.getAge() + 3)).toList();
        newAgeHumans.forEach(System.out::println);
        System.out.println("-----------------");
        double avgAge = newAgeHumans.stream().mapToInt(Human::getAge).average().orElse(0);
